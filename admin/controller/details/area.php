...
class ControllerDetailsArea extends Controller {
	...
	public function add() {
      ...
      $this->load->model('details/area');
              $count=$this->model_details_area->getTotalAreaCount($this->request->post['name']);
              if($count<1)
                {
   $this->model_details_area->addarea($this->request->post);
	...
	protected function getList() {
        ...
        $total_area =$this->model_details_area->getTotalList($filter_data);
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($total_area) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($total_area - $this->config->get('config_limit_admin'))) ? $total_area : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $total_area, ceil($total_area / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('details/area/add', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('details/area/edit', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'], 'SSL');
		}

		$data['cancel'] = $this->url->link('details/area', 'token=' . $this->session->data['token'], 'SSL');
    ...
    /*public function validate() {
		...
		/*if ($this->request->post['area_name'] == '') {
			$this->error['name'] = $this->language->get('error_name');
		}*/
		...
		/*if ($this->request->post['area'] == '') {
			$this->error['area'] = $this->language->get('error_default');
		}
        if ((utf8_strlen($this->request->post['area']) < 3) || (utf8_strlen($this->request->post['area']) > 128)) {
			$this->error['name'] = $this->language->get('error_name');
		}*/
