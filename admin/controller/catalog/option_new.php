<?php
class ControllerCatalogOptionNew extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/option_new');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('catalog/option_new');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/option_new', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/option_new', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		
		$data['add'] = $this->url->link('catalog/option_new/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/option_new/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$option_total = $this->model_catalog_option_new->getTotalOptions();

		$results = $this->model_catalog_option_new->getOptions($filter_data);
		...
		$data['sort_name'] = $this->url->link('catalog/option_new', 'token=' . $this->session->data['token'] . '&sort=od.name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/option_new', 'token=' . $this->session->data['token'] . '&sort=o.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/option_new', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		$data['entry_vendor_description'] ="Enter Vendor Name";
        ...
        if (isset($this->error['vendor_value'])) {
			$data['error_vendor_value'] = $this->error['vendor_value'];
		} else {
			$data['error_vendor_value'] = array();
		}
		...
		if (!isset($this->request->get['option_id'])) {
			$data['action'] = $this->url->link('catalog/option_new/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/option_new/edit', 'token=' . $this->session->data['token'] . '&option_id=' . $this->request->get['option_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/option_new', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['option_value'])) {
            ...
            $vendor_values= $this->request->post['vendor_value'];
       ...
       $data['vendor_values'] = array();
        	foreach ($vendor_values as $vendor_value) {
            	$data['vendor_values'][]=array(
                   'vendor_description'        =>$vendor_value['vendor_description'],  
                );
            }
		...
		$this->response->setOutput($this->load->view('catalog/option_form_new.tpl', $data));
	}
			...
			foreach ($this->request->post['option_value'] as $option_value_id => $option_value) {
				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					if ((utf8_strlen($option_value_description['name']) < 1) || (utf8_strlen($option_value_description['name']) > 128)) {
                        ...
                        echo $this->error['option_value'][$option_value_id][$language_id];
					}
				}
			}
		}

        if (isset($this->request->post['vendor_value'])) {
			foreach ($this->request->post['vendor_value'] as $option_value_id => $option_value) {
                if(empty($option_value['vendor_description']) || $option_value['vendor_description']=='')
                {
                   echo $this->error['vendor_value'][$option_value_id]= "some error";
                }
            }
        }
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'catalog/option_new')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
