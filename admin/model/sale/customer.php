...
class ModelSaleCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
       ...
       // echo $pass;die;
      $this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname='$cust_name',telephone='$cust_no',email='$email',address='$street',city_id='$city',state_id='$state',pincode='$pin',password='$pass',approved='1',status='1' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function deleteCustomer($customer_id) {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "booking_status WHERE customer_id = '" . (int)$customer_id . "'");
	}
    ...
    public function allCustomer($customer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer LEFT JOIN " . DB_PREFIX . "city ON " . DB_PREFIX . "customer.city_id=" . DB_PREFIX . "city.city_id LEFT JOIN " . DB_PREFIX . "zone ON " . DB_PREFIX . "customer.state_id=" . DB_PREFIX . "zone.zone_id where customer_id='$customer_id'");
       //print_r($query);die;
		return $query->row;
	}
...
public function getCustomers($data = array()) {
       // print_r($data);die;
		//$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "city ON " . DB_PREFIX . "customer.city_id=" . DB_PREFIX . "city.city_id LEFT JOIN " . DB_PREFIX . "zone ON " . DB_PREFIX . "customer.state_id=" . DB_PREFIX . "zone.zone_id  WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
        	$sql =("SELECT * FROM " . DB_PREFIX . "customer LEFT JOIN " . DB_PREFIX . "city ON " . DB_PREFIX . "customer.city_id=" . DB_PREFIX . "city.city_id LEFT JOIN " . DB_PREFIX . "zone ON " . DB_PREFIX . "customer.state_id=" . DB_PREFIX . "zone.zone_id ");
...
//echo $data['filter_username'];die;
		if (!empty($data['filter_username'])) {
			$implode[] = "username = '" . $this->db->escape($data['filter_username']) . "'";
		}

		if (!empty($data['filter_city'])) {
			$implode[] = "city_name = '" .$this->db->escape($data['filter_city'])  . "'";
		}

		if (isset($data['filter_address']) && !is_null($data['filter_address'])) {
			$implode[] = "address = '" .$this->db->escape($data['filter_address'])  . "'";
		}

		if (isset($data['filter_mobile']) && !is_null($data['filter_mobile'])) {
			$implode[] = "telephone = '" . $this->db->escape($data['filter_mobile']) . "'";
		}
   

		if (isset($data['filter_state']) && !is_null($data['filter_state'])) {
			$implode[] =  "name LIKE '" . $this->db->escape($data['filter_state']) . "'";
		}
	...
	public function getTotalCustomers($data = array()) {
       ...
       // die;
		...
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer where DATE_FORMAT(date_added,'%Y-%m-%d')='$today_date'";
          ///print_r($sql);die;
		/*$implode = array();
		...
		if (!empty($data['filter_city'])) {
			$implode[] = "customer_id IN (SELECT customer_id FROM " . DB_PREFIX . "customer_ip WHERE ip = '" . $this->db->escape($data['filter_city']) . "')";
		}
		...
		if (!empty($data['filter_state'])) {
			$implode[] = "DATE(date_added) = DATE('" . $this->db->escape($data['filter_state']) . "')";
		}
