<?php
class ModelDetailsTelecallerVendor extends Model {
	

	public function getAllVehicleList($data = array()) {
      //echo "hello";die;
		$sql ="SELECT * FROM `" . DB_PREFIX . "vehicle_description` v,`" . DB_PREFIX . "area` a,`" . DB_PREFIX . "subarea` s,`" . DB_PREFIX . "carmake` c,`" . DB_PREFIX . "carmodel` m,`" . DB_PREFIX . "vehicle_type` vt,`" . DB_PREFIX . "city` ci WHERE a.area_id=v.area AND s.subarea_id=v.sub_area AND c.carmake_id=v.make AND m.carmodel_id=v.model AND vt.vehicle_type_id=v.vehicle_type AND ci.city_id=v.city";

		if (isset($data['filter_name']) && !is_null($data['filter_name'])) {
			$sql .= " AND a.area_name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}

		if (isset($data['filter_address']) && !is_null($data['filter_address'])) {
			$sql .= " AND s.subarea_name LIKE '" . $this->db->escape($data['filter_address']) . "%'";
		}
        
          if (isset($data['filter_number']) && !is_null($data['filter_number'])) {
          $sql .= " AND vt.vehicle_type_name LIKE '" . $this->db->escape($data['filter_number']) . "%'";
		}
        
          if (isset($data['filter_make']) && !is_null($data['filter_make'])) {
          $sql .= " AND c.carmake_name LIKE '" . $this->db->escape($data['filter_make']) . "%'";
		}
         if (isset($data['filter_model']) && !is_null($data['filter_model'])) {
          $sql .= " AND m.carmodel_name LIKE '" . $this->db->escape($data['filter_model']) . "%'";
		}
         if (isset($data['filter_labour']) && !is_null($data['filter_labour'])) {
          $sql .= " AND v.labour LIKE '" . $this->db->escape($data['filter_labour']) . "%'";
		}
		$sort_data = array(
            'a.area_name',
			's.subarea_name',
	...
	public function getTotalList($data = array()) {
			$sql ="SELECT COUNT(vehicle_description_id) AS total FROM `" . DB_PREFIX . "vehicle_description` v";
