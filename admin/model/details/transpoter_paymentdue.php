<?php
class ModelDetailsTranspoterPaymentdue extends Model {
	

	public function getAllAsignedList($data = array()) {
      //$data['customer_id'];
		$sql ="SELECT bs.transporter_id as transporter_id,price_of_transpoter,amount_calc,paid_payment,balance_payment,booking_id FROM " . DB_PREFIX . "payment p," . DB_PREFIX . "booking_status bs," . DB_PREFIX . "vendor v WHERE p.booking_id=bs.booking_status_id And bs.transporter_id=v.vendor_id";
      ...
      if (isset($data['filter_total_balance']) && !is_null($data['filter_total_balance'])) {
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			$sql .= " GROUP BY bs.transporter_id";
		}
