...
function calculateAndDisplayRoute(directionsService, directionsDisplay) {
  ...
  document.getElementById('total').innerHTML = Math.round(total) + ' km'; 
    ...
    var labour_price="<?php echo $vehicle_type['labour_price'];?>";
    var vehicle_price="<?php echo $vehicle_type['vehicle_type_price'];?>";
    var vehicle_base_fair_km="<?php echo $vehicle_type['vehicle_base_fair'];?>";
    var vehicle_base_price="<?php echo $vehicle_type['vehicle_base_km'];?>";

            $.ajax({
				url: 'index.php?route=common/direction_map/statusDone&labour_price='+labour_price+'&vehicle_price='+vehicle_price+'&vehicle_base_fair_km='+vehicle_base_fair_km+'&vehicle_base_price='+vehicle_base_price,
				type: 'post',
				dataType: 'html',
				data: 'km='+total,
				beforeSend: function() {
				...
				success: function(data) {
                    document.getElementById('price').innerHTML ='Rs. '+data+'<b style="font-size:18px;">*</b>';
                     document.getElementById('distance_price').value ='Rs. '+data+'<b style="font-size:18px;">*</b>';
...
var ret_date=$('#ret-date').val();
                   ...
                   if(customer_name=='' || customer_name==null)
                    ...
                    else if(!customer_name.match(/^[a-zA-Z\s]+$/))
                    {
                     ...
                     else if(ret_date=='' || ret_date==null)
                    {
                        alert("Please Enter Date And Time");
							...
							var result=confirm("Are You Sure You Want To Confirm The Booking?");
    if(result==true)
    {
                    form.submit();
    }
	...
	<main class="main" role="main" onload="initMAP()">
		<!-- Page info -->
...
?>
				<aside class="one-fourth sidebar right" style="background:#1493A4">
                   ...
                   ?>
                    ...
                    <div class="full-width">
                        <h2 style="text-align:center;padding:10px;color:#fff">Book Here</h2>
                    ...
                    <tr><th> Service Type </th><td colspan=2><?php if(isset($vehicle_type['vehicle_type_name']))
                                                    echo $vehicle_type['vehicle_type_name'];?> <input type="hidden" name="from" value="<?php echo $from;?>"></td></tr>
                    <tr><th>Approx. Distance (Km)</th><td colspan=2><div id='total'></div><input type="hidden" name="distance" id="distance"></td></tr>
    ...
    <input type="hidden" name="service_type" value="<?php echo $service_type;?>"/>
                    ...
                    <table>
                    <tr><th> Name </th><td colspan=1><input type="text" name="customer_name" id="customer_name" autofocus="autofocus"></td></th></tr>
                     <tr><th> Exact Pickup Location </th><td colspan=1><input type="text" name="exactpickuplocation" id="exactpickuplocation"></td></th></tr>
            
                <tr><th> Mobile No </th><td colspan=1><input type="text" maxlength="10" minlength="10" name="email_id" class="mobile" id="mobile_no" value=""></td></th></tr>
 <tr><th> Date And Time </th><td colspan=1>
		<input type="text" name="ret_date" id="ret-date" class="form-group datepicker one-third"  value="" readonly/>     
                       ...
                       <td style="background:none;border:none;"><a href="<?php echo $back; ?>" class="btn black" data-toggle="tooltip" title="">Back</td>  
